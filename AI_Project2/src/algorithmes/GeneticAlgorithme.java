	...
	public void solve() {
		...
		while (generationNumber < numberOfGeneration && !optimal) {
			...
			for(S s:population){
				...
				if(problem.objective_function(s) > problem.objective_function(best))
					best = s;
				if(problem.objective_function(s) < problem.objective_function(worst))
					worst = s;
				if(problem.goal(s))
					optimal = true;
				//System.out.println(problem.objective_function(s));
			}
			System.out.println("----------------------------------------------------------");
			System.out.println(generationNumber);
			System.out.println("Avg: "+(double)sum/numberOfPopulation);
			System.out.print("best  is:\t");
			problem.print(best);
			System.out.println("\t\twith fitness:\t "+problem.objective_function(best));
	...
	private void selectBests() {
		population.sort(new Comparator<S>() {
			...
			public int compare(S arg0, S arg1) {
				if(problem.objective_function(arg1) > problem.objective_function(arg0))
					return 1;
				else if(problem.objective_function(arg1) < problem.objective_function(arg0))
					return -1;
				else
					return 0;
		...
		for (int i = numberOfPopulation; i < population.size(); i++)
